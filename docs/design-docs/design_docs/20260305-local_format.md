...
`UntilCellExhausted` policy may retain the current Cell pin across adjacent
calls. A forward seek releases that optional pin before the next request.
Neither policy permits a Vortex reader or decoded Arrow batch to be cached in
the public cursor.
