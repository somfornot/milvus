			...
			Policies: []*rootcoordpb.RLSPolicyInfo{
				{
					...
					PolicyType: int32(milvuspb.RowPolicyType_RowPolicyTypePermissive),
					Actions:    []int32{int32(milvuspb.RowPolicyAction_Search)},
					UsingExpr:  "id == 1",
				},
			},
		}, nil).Once()
...
func TestHandleIfSearchByPK_RLSForceRejectsSkip(t *testing.T) {
	mockey.PatchConvey("TestHandleIfSearchByPK_RLSForceRejectsSkip", t, func() {
		...
		_, err := (&Proxy{}).handleIfSearchByPK(ctx, req)
