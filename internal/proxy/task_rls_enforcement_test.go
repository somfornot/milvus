...
func installRLSOperationTestCache(t *testing.T, collectionID int64, schema *schemaInfo, rlsForce ...bool) {
...
func TestRLSForceRejectsSkipAcrossOperations(t *testing.T) {
	...
	t.Run("search", func(t *testing.T) {
		...
		ctx := context.Background()
		installRLSOperationTestCache(t, collectionID, newRLSOperationTestSchema(collectionName), true)
		searchParams := getValidSearchParams()
		resetSearchParamsValue(searchParams, AnnsFieldKey, "vector")
		task := &searchTask{
			ctx:            ctx,
