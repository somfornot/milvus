...
class PhyCompareFilterExpr : public Expr {
 ...
 private:
    ...
    ProcessBothDataByOffsetsByTake(FUNC func,
        ...
        if (left_take->IsOwned() && right_take->IsOwned()) {
            ...
            for (int64_t i = 0; i < size; ++i) {
                if (!left_take->IsValid(i) || !right_take->IsValid(i)) {
                    res[i] = false;
                    valid_res[i] = false;
                }
            }
            ...
            for (int64_t i = 0; i < size; ++i) {
                ...
                const auto left = left_take->Get<T>(i);
                const auto right = right_take->Get<U>(i);
                func.template operator()<FilterType::random>(
        ...
        if (left_data_scan_cursor_ == nullptr) {
            ...
            auto left_cursor = left_data_column_->Scan(
                ...
                ChunkedColumnInterface::ScanOptions::ForData(
                    window_start, DataScanValueKind<T>(), pin_policy));
            auto right_cursor = right_data_column_->Scan(
                op_ctx_,
                ChunkedColumnInterface::ScanOptions::ForData(
                    window_start, DataScanValueKind<U>(), pin_policy));
