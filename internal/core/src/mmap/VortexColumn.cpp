...
class VortexColumnPlanner final : public ColumnPlanner {
 public:
    VortexColumnPlanner(std::vector<int64_t> num_rows_until_file,
                        const std::shared_ptr<VortexColumnGroup>& column_group,
                        std::string_view field_name)
        : ColumnPlanner(std::move(num_rows_until_file)) {
        AssertInfo(column_group != nullptr,
                   "vortex Column planner has no ColumnGroup");
        const auto& files = column_group->files();
        ...
        for (size_t file_id = 0; file_id < files.size(); ++file_id) {
            file_planner_delegates_.emplace_back(
                column_group->FieldPlanner(file_id, field_name));
        }
        AssertInfo(
...
GetVortexPlanner(const VortexColumn& column) {
    return static_cast<const VortexColumnPlanner&>(column.Planner());
}
...
class VortexTakeResult final : public ChunkedColumnInterface::TakeResult {
 public:
    ...
    IsValid(int64_t index) const override {
        CheckIndex(index);
        return validity_ == nullptr || validity_[index];
    }
    ...
    GetOwn() const override {
        return ChunkedColumnInterface::OwnedTakeData{
            values_, validity_, owner_, size_};
    }
 ...
 protected:
    ...
    FixedValueAt(int64_t index) const override {
        CheckIndex(index);
        AssertInfo(values_.encoding == ValueEncoding::FixedWidth &&
                       values_.data != nullptr && values_.byte_width > 0,
                   "invalid vortex fixed-width take result");
    ...
    StringViewAt(int64_t index) const override {
        CheckIndex(index);
        return values_.data_as<std::string_view>()[index];
    }

    Json
    JsonAt(int64_t index) const override {
        CheckIndex(index);
        return values_.data_as<Json>()[index];
    }

    ArrayView
    ArrayAt(int64_t index) const override {
        CheckIndex(index);
        return values_.data_as<ArrayView>()[index];
    }

 private:
    void
    CheckIndex(int64_t index) const {
        AssertInfo(index >= 0 && index < size_,
                   "vortex take index {} out of range {}",
                   index,
                   size_);
    }
...
class VortexDataScanCursor final {
        ...
        while (scan_pos_ < scan_end_) {
            ...
            if (reader_.has_value()) {
                ...
                if (batch != nullptr) {
                    ...
                    current_array_ =
                        PrepareBatchArray(current_batch_->column(0));
 ...
 private:
    ...
    bool
    IsArrayScan() const {
        return value_kind_ == ChunkedColumnInterface::ScanValueKind::ArrayView;
    }
    ...
    PrepareBatchArray(const std::shared_ptr<arrow::Array>& array) const {
        ...
        if (column_->SupportsDirectDataScan()) {
            ValidateVortexArrowArray(column_->field_id_,
                                     column_->data_type_,
                                     column_->IsNullable(),
                                     array,
    ...
    FillArrayOutput(const std::shared_ptr<arrow::Array>& array,
        ...
        for (int64_t i = 0; i < out->size; ++i) {
            const auto valid = !emit_valid || views.second[i];
            if (!valid) {
                owner->arrays.emplace_back();
                continue;
            }
            ScalarFieldProto proto;
            const auto& value = views.first[i];
        ...
        for (int64_t i = 0; i < out->size; ++i) {
            if (emit_valid && !views.second[i]) {
                continue;
            }
            auto& value = owner->arrays[i];
            owner->array_views[i] = ArrayView(const_cast<char*>(value.data()),
                                              value.length(),
                                              value.byte_size(),
    ...
    std::shared_ptr<arrow::Array> current_array_;
 ...
 public:
    VortexScanCursor(const VortexColumn* column,
        ...
        if (output_ == ChunkedColumnInterface::ScanOutput::Data) {
            ...
            value_kind_ = ResolveDataScanValueKind(column_->data_type_,
                                                   options.value_kind);
        } else {
    ...
    Next(int64_t position,
        ...
        AssertInfo(
            length >= 0 &&
                position <= static_cast<int64_t>(column_->NumRows()) &&
                length <= static_cast<int64_t>(column_->NumRows()) - position,
        ...
        return ReadData(position, length, mode, out);
    }

 private:
    VortexCellPin
    PinCells(int64_t chunk_id, const std::vector<uint64_t>& cell_ids) const {
        if (sticky_pin_ != nullptr && sticky_chunk_id_ == chunk_id &&
            sticky_cell_ids_ == cell_ids) {
            return sticky_pin_;
        }
    ...
    ReadData(int64_t position,
        ...
        auto range =
            NextVortexReaderRange(column_, &range_pos, position + length);
        ...
        std::vector<VortexDataScanSource> sources;
        sources.emplace_back(VortexDataScanSource{*range, std::move(reader)});
        VortexDataScanCursor data_reader(column_,
                                         position,
                                         range->length,
                                         mode,
                                         value_kind_,
                                         std::move(sources));
        const auto returned = data_reader.NextBatch(range->length, out);
        ...
        RememberStickyPin(range->chunk_id, plan.cell_ids, pin);
        return true;
    }
    ...
    ReadRowIds(int64_t position,
        ...
        while (auto range =
                   NextVortexReaderRange(column_, &range_pos, scan_end)) {
    ...
    void
    RememberStickyPin(int64_t chunk_id,
    ...
    int64_t scan_pos_{0};
    VortexCellPin sticky_pin_;
    int64_t sticky_chunk_id_{-1};
    std::vector<uint64_t> sticky_cell_ids_;
};
...
VortexColumn::BuildPlanner() const {
    return std::make_unique<VortexColumnPlanner>(
        column_group_->num_rows_until_chunk(),
        column_group_,
        field_name_);
}
...
VortexColumn::PrefetchChunks(milvus::OpContext* op_ctx,
    ...
    for (auto chunk_id : chunk_ids) {
        ...
        const auto num_cells =
            GetVortexPlanner(*this).FileDelegate(chunk_id).num_cells();
...
VortexColumn::StringViewsByOffsets(milvus::OpContext* op_ctx,
    ...
    if (data_type_ == DataType::JSON) {
        for (size_t i = 0; i < offsets.size(); ++i) {
            views.first[i] = take->Get<Json>(i);
            views.second[i] = take->IsValid(i);
        }
    } else {
        for (size_t i = 0; i < offsets.size(); ++i) {
            views.first[i] = take->Get<std::string_view>(i);
            views.second[i] = take->IsValid(i);
        }
    }
...
VortexColumn::ArrayViewsByOffsets(milvus::OpContext* op_ctx,
    ...
    AssertInfo(take->Size() == static_cast<int64_t>(offsets.size()),
               ...
               offsets.size());
    for (size_t i = 0; i < offsets.size(); ++i) {
        views.first[i] = take->Get<ArrayView>(i);
        views.second[i] = take->IsValid(i);
    }
    auto owned = take->GetOwn();
    return PinWrapper<std::pair<std::vector<ArrayView>, FixedVector<bool>>>(
...
VortexColumn::ApplyValidDataInChunk(milvus::OpContext* op_ctx,
    ...
    auto cursor = Scan(op_ctx, ScanOptions::ForData(global_start));
    ...
    while (processed < size) {
        ScanBatch batch;
        const auto returned = cursor->Next(global_start + processed,
                                           size - processed,
                                           ScanReadMode::ValidityOnly,
                                           &batch);
        AssertInfo(returned,
...
VortexColumn::BulkValueAt(milvus::OpContext* op_ctx,
    ...
    AssertInfo(take->Size() == count,
               ...
               count);
    auto owned = take->GetOwn();
    AssertInfo(owned.values.encoding == ValueEncoding::FixedWidth,
               "vortex bulk value expected fixed-width take, got {}",
               static_cast<int>(owned.values.encoding));
...
VortexColumn::ValidateFileState(
    const VortexColumnGroup::FileState& group_file,
    const milvus_storage::vortex::VortexPlanner& planner) const {
    AssertInfo(static_cast<int64_t>(planner.rows()) == group_file.rows,
               ...
               group_file.rows,
               group_file.path);
    AssertInfo(planner.num_cells() == group_file.slot->num_cells(),
    ...
    const auto& field_cells = planner.cell_metas();
...
VortexColumn::BuildFileState(
    ...
    const auto& column_planner = GetVortexPlanner(*this);
    const auto& file_planner = column_planner.FileDelegate(file_index);
    ValidateFileState(group_file, file_planner);
    AssertInfo(static_cast<int64_t>(file_planner.rows()) ==
                   column_planner.CellRows(file_index),
               "vortex field {} file {} rows {} do not match Column planner "
               "rows {}",
               field_id_.get(),
               file_index,
               file_planner.rows(),
...
VortexColumn::PlanRowRange(int64_t file_id,
                           ...
                           uint64_t row_end,
                           const std::string& predicate) const {
    return GetVortexPlanner(*this).PlanRowRange(
...
VortexColumn::PlanOffsets(int64_t file_id,
    ...
    return GetVortexPlanner(*this).PlanOffsets(file_id, offsets);
}
...
VortexColumn::TakeArrowFromFile(milvus::OpContext* op_ctx,
    ...
    result.table = std::move(table_result).ValueOrDie();
...
VortexColumn::Take(milvus::OpContext* op_ctx,
    ...
    const auto requested_kind = options.value_kind;
    ...
    const auto value_kind = requested_kind == ScanValueKind::Default
                                ? expected_kind
                                : requested_kind;
    ...
    std::sort(entries.begin(),
              ...
              [](const OffsetEntry& left, const OffsetEntry& right) {
                  return std::tie(left.segment_offset,
    ...
    if (IsNullable()) {
        owner->validity.resize(offsets.size);
    ...
    return std::make_unique<VortexTakeResult>(
        values, validity, std::move(owner), offsets.size, data_type_);
}
...
VortexColumn::BulkStringLikeAt(
    ...
    if (offsets == nullptr) {
        ...
        if (!SupportsDirectDataScan()) {
            ...
            for (int64_t chunk_id = 0; chunk_id < num_chunks(); ++chunk_id) {
                ...
                global_offset += planner.CellRows(chunk_id);
            }
            return;
        }
        int64_t global_offset = 0;
    ...
    if (data_type_ == DataType::JSON) {
        for (int64_t i = 0; i < count; ++i) {
            fn(take->Get<Json>(i), i, take->IsValid(i));
        }
    } else {
        for (int64_t i = 0; i < count; ++i) {
            fn(take->Get<std::string_view>(i), i, take->IsValid(i));
        }
    }
}
