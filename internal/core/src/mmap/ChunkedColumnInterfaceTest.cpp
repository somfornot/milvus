...
TYPED_TEST(VectorArrayColumnInterfaceTest,
    ...
    auto cursor = fx.column->Scan(
        nullptr, ChunkedColumnInterface::ScanOptions::ForData(0));
    ASSERT_NE(cursor, nullptr);

    ChunkedColumnInterface::ScanBatch batch;
    ASSERT_TRUE(cursor->Next(
        0, 4, ChunkedColumnInterface::ScanReadMode::DataAndValidity, &batch));
...
TYPED_TEST(VectorArrayColumnInterfaceTest,
    ...
    ChunkedColumnInterface::ScanBatch batch;
    ASSERT_TRUE(cursor->Next(
        0, 2, ChunkedColumnInterface::ScanReadMode::ValidityOnly, &batch));
    ...
    ASSERT_TRUE(cursor->Next(
        2, 2, ChunkedColumnInterface::ScanReadMode::ValidityOnly, &batch));
...
TYPED_TEST(VectorArrayColumnInterfaceTest, DataScanRejectsMismatchedValueKind) {
    ...
    EXPECT_THROW(fx.column->Scan(
                     nullptr,
                     ChunkedColumnInterface::ScanOptions::ForData(
                         0, ChunkedColumnInterface::ScanValueKind::FixedWidth)),
                 std::exception);
}
...
TYPED_TEST(ChunkedColumnInterfaceTest,
    ...
    auto cursor = fx.column->Scan(
        nullptr,
        ChunkedColumnInterface::ScanOptions::ForData(
            1, ChunkedColumnInterface::ScanValueKind::FixedWidth));
    ASSERT_NE(cursor, nullptr);
    ...
    ChunkedColumnInterface::ScanBatch batch;
    ASSERT_TRUE(cursor->Next(
        1, 4, ChunkedColumnInterface::ScanReadMode::DataAndValidity, &batch));
    ...
    EXPECT_TRUE(IsScanRowValid(batch, 1));
    ASSERT_TRUE(cursor->Next(
        3, 2, ChunkedColumnInterface::ScanReadMode::DataAndValidity, &batch));
...
TYPED_TEST(ChunkedColumnInterfaceTest,
    ...
    ChunkedColumnInterface::ScanBatch batch;
    ASSERT_TRUE(cursor->Next(
        0, 4, ChunkedColumnInterface::ScanReadMode::ValidityOnly, &batch));
    EXPECT_EQ(batch.row_id_start, 0);
    EXPECT_EQ(batch.size, 4);
    EXPECT_TRUE(batch.values.empty());
    EXPECT_TRUE(IsScanRowValid(batch, 0));
...
TYPED_TEST(ChunkedColumnInterfaceTest,
           FixedWidthDataScanPinsEachCallByDefault) {
    ...
    ChunkedColumnInterface::ScanBatch batch;
    ASSERT_TRUE(cursor->Next(
        1, 2, ChunkedColumnInterface::ScanReadMode::DataAndValidity, &batch));
    EXPECT_EQ(batch.row_id_start, 1);
    EXPECT_EQ(batch.size, 2);

    ASSERT_TRUE(cursor->Next(
        3, 1, ChunkedColumnInterface::ScanReadMode::DataAndValidity, &batch));
    EXPECT_EQ(batch.row_id_start, 3);
    EXPECT_EQ(batch.size, 1);

    ASSERT_TRUE(cursor->Next(
        4, 1, ChunkedColumnInterface::ScanReadMode::DataAndValidity, &batch));
...
TYPED_TEST(ChunkedColumnInterfaceTest,
           FixedWidthDataScanCanKeepCurrentCellPinnedAcrossCalls) {
    ColumnSpec spec{{5}, {}, /*nullable=*/false};
    spec.data_type = DataType::INT32;
    auto fx = TypeParam::Create(spec);
    ...
    ChunkedColumnInterface::ScanBatch batch;
    ASSERT_TRUE(cursor->Next(
        0, 2, ChunkedColumnInterface::ScanReadMode::DataAndValidity, &batch));
    ASSERT_EQ(fx.pin_requests->size(), 1u);
    EXPECT_EQ(fx.pin_requests->front(), (std::vector<int64_t>{0}));

    ASSERT_TRUE(cursor->Next(
        2, 2, ChunkedColumnInterface::ScanReadMode::DataAndValidity, &batch));
    EXPECT_EQ(fx.pin_requests->size(), 1u);
...
TYPED_TEST(ChunkedColumnInterfaceTest, StickyScanReturnsBeforeCrossingCells) {
    ...
    ASSERT_TRUE(cursor->Next(
        3, 3, ChunkedColumnInterface::ScanReadMode::DataAndValidity, &batch));
    EXPECT_EQ(batch.row_id_start, 3);
    EXPECT_EQ(batch.size, 3);
    EXPECT_EQ(fx.pin_requests->size(), 2u);
    EXPECT_EQ((*fx.pin_requests)[1], (std::vector<int64_t>{1}));
...
TYPED_TEST(ChunkedColumnInterfaceTest,
           ScanSkipsZeroRowCellsAtBoundaries) {
    ...
    ChunkedColumnInterface::ScanBatch batch;
    ASSERT_TRUE(cursor->Next(
        0, 5, ChunkedColumnInterface::ScanReadMode::DataAndValidity, &batch));
    EXPECT_EQ(batch.row_id_start, 0);
    EXPECT_EQ(batch.size, 2);
    ...
    ASSERT_TRUE(cursor->Next(
        2, 3, ChunkedColumnInterface::ScanReadMode::DataAndValidity, &batch));
    EXPECT_EQ(batch.row_id_start, 2);
    EXPECT_EQ(batch.size, 3);
...
TYPED_TEST(ChunkedColumnInterfaceTest,
           StickyScanSkipsZeroRowCellsWithoutRepinning) {
    ColumnSpec spec{{0, 0, 2, 0, 0, 3, 0, 0}, {}, /*nullable=*/false};
    spec.data_type = DataType::INT32;
    auto fx = TypeParam::Create(spec);
    ...
    ChunkedColumnInterface::ScanBatch batch;
    ASSERT_TRUE(cursor->Next(
        0, 1, ChunkedColumnInterface::ScanReadMode::DataAndValidity, &batch));
    ASSERT_TRUE(cursor->Next(
        1, 1, ChunkedColumnInterface::ScanReadMode::DataAndValidity, &batch));
    ASSERT_TRUE(cursor->Next(
        2, 3, ChunkedColumnInterface::ScanReadMode::DataAndValidity, &batch));
    EXPECT_EQ(batch.row_id_start, 2);
    EXPECT_EQ(batch.size, 3);
...
TYPED_TEST(ChunkedColumnInterfaceTest,
           AbsoluteNextPositionSeeksWithoutPinningSkippedCells) {
    ...
    ASSERT_TRUE(cursor->Next(
        5, 1, ChunkedColumnInterface::ScanReadMode::DataAndValidity, &batch));
    ASSERT_EQ(fx.pin_requests->size(), 2u);
    EXPECT_EQ((*fx.pin_requests)[1], (std::vector<int64_t>{1}));
}

TYPED_TEST(ChunkedColumnInterfaceTest, DataScanRejectsBackwardPosition) {
...
TYPED_TEST(ChunkedColumnInterfaceTest, NonNullableScanRejectsValidityOnlyMode) {
    ...
    EXPECT_THROW(
        cursor->Next(
            0, 3, ChunkedColumnInterface::ScanReadMode::ValidityOnly, &batch),
        std::exception);
    EXPECT_TRUE(fx.fetched->empty());
    EXPECT_TRUE(fx.pin_requests->empty());
}
...
TYPED_TEST(ChunkedColumnInterfaceTest,
    ...
    ChunkedColumnInterface::ScanBatch batch;
    ASSERT_TRUE(cursor->Next(
        0, 3, ChunkedColumnInterface::ScanReadMode::DataAndValidity, &batch));
...
TYPED_TEST(ChunkedColumnInterfaceTest,
    ...
    for (int64_t i = 0; i < take->Size(); ++i) {
        actual.emplace_back(take->template Get<int32_t>(i));
...
TYPED_TEST(ChunkedColumnInterfaceTest, TakeConsumesOrderedCellLocations) {
    ColumnSpec spec{{3, 2}, {}, /*nullable=*/false};
    spec.data_type = DataType::INT32;
    auto fx = TypeParam::Create(spec);

    const FixedVector<int32_t> offsets{4, 1, 3, 1};
    auto plan = fx.column->Planner().PlanTake(
        ChunkedColumnInterface::OffsetView::From(
            offsets.data(), static_cast<int64_t>(offsets.size())));
    ASSERT_EQ(plan.Size(), static_cast<int64_t>(offsets.size()));
    EXPECT_EQ(plan.locations[0].source_cell_id, 1);
    EXPECT_EQ(plan.locations[0].cell_offset, 1u);
    EXPECT_EQ(plan.locations[1].source_cell_id, 0);
    EXPECT_EQ(plan.locations[1].cell_offset, 1u);
    EXPECT_EQ(plan.locations[2].source_cell_id, 1);
    EXPECT_EQ(plan.locations[2].cell_offset, 0u);
    EXPECT_EQ(plan.locations[3].source_cell_id, 0);
    EXPECT_EQ(plan.locations[3].cell_offset, 1u);
    EXPECT_TRUE(fx.pin_requests->empty());

    auto take =
        fx.column->Take(nullptr,
                        std::move(plan),
                        ChunkedColumnInterface::ScanValueKind::FixedWidth);
    ASSERT_NE(take, nullptr);
    ASSERT_EQ(take->Size(), static_cast<int64_t>(offsets.size()));
...
TYPED_TEST(ChunkedColumnInterfaceTest,
           ColumnPlannerOwnsTakeAndScanCellConversion) {
    ColumnSpec spec{{3, 2}, {}, /*nullable=*/false};
    spec.data_type = DataType::INT32;
    auto fx = TypeParam::Create(spec);

    const auto& planner = fx.column->Planner();
    EXPECT_EQ(&planner, &fx.column->Planner());
    EXPECT_EQ(planner.NumCells(), 2);
    EXPECT_EQ(planner.NumRows(), 5);
    EXPECT_EQ(planner.CellBoundaries(), (std::vector<int64_t>{0, 3, 5}));

    const FixedVector<int32_t> offsets{4, 1, 3, 1};
    auto take_plan = planner.PlanTake(
        ChunkedColumnInterface::OffsetView::From(
            offsets.data(), static_cast<int64_t>(offsets.size())));
    ASSERT_EQ(take_plan.Size(), 4);
    EXPECT_EQ(take_plan.locations[0].source_cell_id, 1);
    EXPECT_EQ(take_plan.locations[0].cell_offset, 1u);
    EXPECT_EQ(take_plan.locations[1].source_cell_id, 0);
    EXPECT_EQ(take_plan.locations[1].cell_offset, 1u);
    EXPECT_EQ(take_plan.locations[2].source_cell_id, 1);
    EXPECT_EQ(take_plan.locations[2].cell_offset, 0u);
    EXPECT_EQ(take_plan.locations[3].source_cell_id, 0);
    EXPECT_EQ(take_plan.locations[3].cell_offset, 1u);

    std::vector<int> scan_decisions(2, 0);
    auto scan_plan = planner.PlanScan(1, 4, [&](int64_t cell_id) {
        ++scan_decisions[cell_id];
        return cell_id == 0;
    });
    EXPECT_EQ(scan_decisions, (std::vector<int>{1, 1}));
    ASSERT_EQ(scan_plan.cells.size(), 2u);
    EXPECT_EQ(scan_plan.cells[0].cell_id, 0);
    EXPECT_EQ(scan_plan.cells[0].row_start, 1);
    EXPECT_EQ(scan_plan.cells[0].cell_offset, 1);
    EXPECT_EQ(scan_plan.cells[0].row_count, 2);
    EXPECT_TRUE(scan_plan.cells[0].skip_data);
    EXPECT_EQ(scan_plan.cells[1].cell_id, 1);
    EXPECT_EQ(scan_plan.cells[1].row_start, 3);
    EXPECT_EQ(scan_plan.cells[1].cell_offset, 0);
    EXPECT_EQ(scan_plan.cells[1].row_count, 2);
    EXPECT_FALSE(scan_plan.cells[1].skip_data);
    EXPECT_TRUE(fx.pin_requests->empty());
}
...
TYPED_TEST(ChunkedColumnInterfaceTest,
    ...
    fx.column.reset();
    EXPECT_EQ(take->template Get<int32_t>(0), 3);
    EXPECT_EQ(take->template Get<int32_t>(1), 2);
    EXPECT_EQ(take->template Get<int32_t>(2), 0);
    ASSERT_EQ(fx.pin_requests->size(), 2u);
...
TYPED_TEST(ChunkedColumnInterfaceTest,
    ...
    ASSERT_NE(owned.validity, nullptr);
    take.reset();
...
TYPED_TEST(ChunkedColumnInterfaceTest,
    ...
    EXPECT_THROW(fx.column->Scan(
                     nullptr,
                     ChunkedColumnInterface::ScanOptions::ForData(
                         0, ChunkedColumnInterface::ScanValueKind::StringView)),
                 std::exception);
}
...
TEST(ChunkedColumnInterfaceTest, VarcharPrimaryKeyUsesStringViewScanCursor) {
    ...
    auto cursor =
        column->Scan(nullptr, ChunkedColumnInterface::ScanOptions::ForData(0));
    ASSERT_NE(cursor, nullptr);
...
TEST(ChunkedColumnInterfaceTest, VarcharTakeBuildsOnlyRequestedOrderedViews) {
    ...
    ASSERT_EQ(take->Size(), 4);
    EXPECT_TRUE(pin_requests->empty());
    EXPECT_EQ(take->Get<std::string_view>(0), "delta");
    EXPECT_EQ(take->Get<std::string_view>(1), "beta");
    EXPECT_EQ(take->Get<std::string_view>(2), "beta");
    EXPECT_EQ(take->Get<std::string_view>(3), "alpha");
    ASSERT_EQ(pin_requests->size(), 1u);
    EXPECT_EQ(pin_requests->front(), (std::vector<int64_t>{0}));
}
