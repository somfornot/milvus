...
ScanValueKind
ResolveVirtualPKValueKind(ScanValueKind requested) {
...
MakeVirtualPKValueView(const VirtualPKValues& owner) {
    ...
    values.physical_type = DataType::INT64;
    values.logical_type = DataType::INT64;
    values.data = owner.values.data();
    values.offset = 0;
    values.size = static_cast<int64_t>(owner.values.size());
...
VirtualPKChunkedColumn::Scan(milvus::OpContext*,
    ...
    ResolveVirtualPKValueKind(options.value_kind);
...
VirtualPKChunkedColumn::Take(milvus::OpContext*,
    ...
    ResolveVirtualPKValueKind(requested_kind);
    auto owner = std::make_shared<VirtualPKValues>();
