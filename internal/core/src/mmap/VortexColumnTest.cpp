...
CollectIntScanValues(VortexColumn& column, int64_t start, int64_t length) {
    auto options = ChunkedColumnInterface::ScanOptions::ForData(start);
...
CollectStringScanValues(VortexColumn& column, int64_t start, int64_t length) {
    auto options = ChunkedColumnInterface::ScanOptions::ForData(
        start, ChunkedColumnInterface::ScanValueKind::StringView);
...
CollectFilteredRowIdPayload(VortexColumn& column,
    ...
    ChunkedColumnInterface::ScanBatch batch;
    EXPECT_TRUE(
        result->Next(options.start_offset,
                     length,
                     ChunkedColumnInterface::ScanReadMode::DataAndValidity,
                     &batch));
...
CheckNoDataScan(VortexColumn& column) {
    auto options = ChunkedColumnInterface::ScanOptions::ForData(0);
    ...
    auto all_valid_cursor =
        column.Scan(nullptr, ChunkedColumnInterface::ScanOptions::ForData(2));
...
CheckDataScan(VortexColumn& column, DataType type) {
    auto options =
        ChunkedColumnInterface::ScanOptions::ForData(0, ScanKindForType(type));
...
CheckOrderedTake(VortexColumn& column, DataType type) {
    ...
    for (int64_t i = 0; i < take->Size(); ++i) {
        ...
        switch (type) {
            case DataType::BOOL:
                EXPECT_EQ(take->Get<bool>(i), row % 2 == 0) << row;
                break;
            case DataType::INT8:
                EXPECT_EQ(take->Get<int8_t>(i), static_cast<int8_t>(row - 8))
                    << row;
                break;
            case DataType::INT16:
                EXPECT_EQ(take->Get<int16_t>(i), static_cast<int16_t>(row * 10))
                    << row;
                break;
            case DataType::INT32:
                EXPECT_EQ(take->Get<int32_t>(i),
                          static_cast<int32_t>(row * 100))
                    << row;
                break;
            case DataType::INT64:
                EXPECT_EQ(take->Get<int64_t>(i), row * 1000) << row;
                break;
            case DataType::TIMESTAMPTZ:
                EXPECT_EQ(take->Get<int64_t>(i), 1700000000000000LL + row)
                    << row;
                break;
            case DataType::FLOAT:
                EXPECT_NEAR(
                    take->Get<float>(i), static_cast<float>(row) * 1.5f, 1e-5)
                    << row;
                break;
            case DataType::DOUBLE:
                EXPECT_NEAR(
                    take->Get<double>(i), static_cast<double>(row) * 2.25, 1e-9)
                    << row;
                break;
            case DataType::STRING:
            case DataType::VARCHAR:
            case DataType::TEXT:
            case DataType::GEOMETRY:
                EXPECT_EQ(take->Get<std::string_view>(i),
                          ExpectedString(type, row))
                    << row;
                break;
            case DataType::JSON: {
                std::string_view value = take->Get<Json>(i);
                EXPECT_EQ(value, ExpectedString(type, row)) << row;
                break;
            }
            case DataType::ARRAY: {
                const auto value = take->Get<ArrayView>(i);
                if (row == 0) {
...
TEST(VortexColumnTest, ScanAndTake) {
    ...
    column_group->ManualEvictCache();
    EXPECT_FALSE(int_column.CellsLoaded(offsets.data(), offsets.size()));
    EXPECT_EQ(take_result->Get<int32_t>(0), 70);
    EXPECT_EQ(take_result->Get<int32_t>(1), 10);
    EXPECT_EQ(take_result->Get<int32_t>(2), 70);
    EXPECT_EQ(take_result->Get<int32_t>(3), 150);
    take_result.reset();
    column_group->ManualEvictCache();
    EXPECT_FALSE(int_column.CellsLoaded(offsets.data(), offsets.size()));
    ...
    take_result = string_column.Take(
        nullptr,
        ChunkedColumnInterface::TakeOptions{
    ...
    ASSERT_NE(take_result, nullptr);
    EXPECT_EQ(take_result->Get<std::string_view>(0), "v7");
    EXPECT_EQ(take_result->Get<std::string_view>(1), "v1");
    EXPECT_EQ(take_result->Get<std::string_view>(2), "v7");
    EXPECT_EQ(take_result->Get<std::string_view>(3), "v15");
...
TEST(VortexColumnTest, MultiFileTakeAndScan) {
    ...
    const auto& planner = int_column.Planner();
    EXPECT_EQ(&planner, &int_column.Planner());
    EXPECT_EQ(planner.NumRows(), 32);
    EXPECT_EQ(planner.NumCells(), 2);
    EXPECT_EQ(planner.CellBoundaries(), (std::vector<int64_t>{0, 16, 32}));
    EXPECT_EQ(planner.Locate(17).cell_id, 1);
    EXPECT_EQ(planner.Locate(17).cell_offset, 1);

    const auto scan_plan = planner.PlanScan(14, 5);
    ASSERT_EQ(scan_plan.cells.size(), 2);
    EXPECT_EQ(scan_plan.cells[0].cell_id, 0);
    EXPECT_EQ(scan_plan.cells[0].row_start, 14);
    EXPECT_EQ(scan_plan.cells[0].cell_offset, 14);
    EXPECT_EQ(scan_plan.cells[0].row_count, 2);
    EXPECT_EQ(scan_plan.cells[1].cell_id, 1);
    EXPECT_EQ(scan_plan.cells[1].row_start, 16);
    EXPECT_EQ(scan_plan.cells[1].cell_offset, 0);
    EXPECT_EQ(scan_plan.cells[1].row_count, 3);
    ...
    for (int64_t i = 0; i < take_result->Size(); ++i) {
        taken_values.emplace_back(take_result->Get<int32_t>(i));
    }
    ...
    auto all_valid_cursor = int_column.Scan(
        nullptr, ChunkedColumnInterface::ScanOptions::ForData(14));
    ASSERT_NE(all_valid_cursor, nullptr);
    EXPECT_FALSE(int_column.CellsLoaded(first_file_offset, 1));
    EXPECT_FALSE(int_column.CellsLoaded(second_file_offset, 1));
    auto cross_chunk_options = ChunkedColumnInterface::ScanOptions::ForData(14);
...
TEST(VortexColumnTest, RejectsNonContiguousFileRanges) {
    ...
    EXPECT_THROW(MakeColumnGroup(std::move(files),
                                 properties,
                                 {std::to_string(kIntFieldId)}),
                 std::exception);
}
...
TEST(VortexColumnTest, PreservesNormalizationForDataScan) {
    ...
    EXPECT_EQ(timestamp_take->Get<int64_t>(0), timestamp_millis[1] * 1000);
    EXPECT_EQ(timestamp_take->Get<int64_t>(1), timestamp_millis[0] * 1000);
    ...
    EXPECT_EQ(geometry_take->Get<std::string_view>(0),
              Geometry(ctx, geometry_wkt[1].c_str()).to_wkb_string());
    EXPECT_EQ(geometry_take->Get<std::string_view>(1),
              Geometry(ctx, geometry_wkt[0].c_str()).to_wkb_string());

    std::filesystem::remove_all(dir);
}

TEST(VortexColumnTest, RejectsMismatchedScanValueKind) {
    auto schema = MakeSchema();
    auto properties =
        std::make_shared<milvus_storage::api::Properties>(MakeProperties());
    auto dir =
...
TEST(VortexColumnTest, ScanPlansAndPinsOnlyForTheRequestedRange) {
    ...
    EXPECT_FALSE(
        skipped_cursor->Next(kNullableRows,
                             0,
                             ChunkedColumnInterface::ScanReadMode::ValidityOnly,
                             &validity_batch));
    EXPECT_FALSE(column.CellsLoaded(planned_offsets, 2));
    ...
    auto per_call_cursor =
        column.Scan(nullptr, ChunkedColumnInterface::ScanOptions::ForData(0));
    ASSERT_NE(per_call_cursor, nullptr);
    ChunkedColumnInterface::ScanBatch pin_batch;
    ASSERT_TRUE(per_call_cursor->Next(
        0, 2, ChunkedColumnInterface::ScanReadMode::ValidityOnly, &pin_batch));
    pin_batch = {};
    column.ManualEvictCache();
    EXPECT_FALSE(column.CellsLoaded(first_offset, 1));

    auto sticky_options = ChunkedColumnInterface::ScanOptions::ForData(
        0,
        ChunkedColumnInterface::ScanValueKind::Default,
        ChunkedColumnInterface::ScanPinPolicy::UntilCellExhausted);
    auto sticky_cursor = column.Scan(nullptr, sticky_options);
    ASSERT_NE(sticky_cursor, nullptr);
    ASSERT_TRUE(sticky_cursor->Next(
        0, 2, ChunkedColumnInterface::ScanReadMode::ValidityOnly, &pin_batch));
    pin_batch = {};
    column.ManualEvictCache();
    EXPECT_TRUE(column.CellsLoaded(first_offset, 1));
    ...
    ASSERT_TRUE(sticky_cursor->Next(
        2, 2, ChunkedColumnInterface::ScanReadMode::ValidityOnly, &pin_batch));
    pin_batch = {};
    column.ManualEvictCache();
    EXPECT_TRUE(column.CellsLoaded(first_offset, 1));

    // An absolute forward seek releases the retained Cell before handling the
    // next request. A zero-length request performs no planning or pinning.
    EXPECT_FALSE(
        sticky_cursor->Next(kNullableRows,
                            0,
                            ChunkedColumnInterface::ScanReadMode::ValidityOnly,
                            &pin_batch));
    column.ManualEvictCache();
